...
test('add itemmaster after login', async ({ page }) => {
  ...
  await expect(page).toHaveURL('https://swtest.craftsmanautomation.com:8090/wms-milkymist/web/');
  ...
  while (!found) {
    ...
    if (await row.count() > 0) {
      await expect(row).toContainText(uniqueItemCode);
      await expect(row).toContainText('iceragi');
