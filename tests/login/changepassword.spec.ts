...
test.describe('Change Password for First-Time Users', () => {
  ...
  test('Verify first-time user exists in database and has empty LastLoginDateTime/LastPassword', async () => {
    ...
    if (userDetails) {
      ...
      console.log(`\n✓ User "${loginData.firstTimeUser}" is new user:
        - LastLoginDateTime: ${userDetails.LastLoginDateTime || '[EMPTY]'}
        - LastPassword: ${userDetails.LastPassword || '[EMPTY]'}
      `);
    } else {
      console.warn(`⚠ User "${loginData.firstTimeUser}" not found in database. Skipping new user verification.`);
    }
  });
  ...
  test('Full workflow: Database check → Login → Popup validation → Password rules check', async () => {
    ...
    if (userDetails) {
      ...
      expect(isNewUser).toBeTruthy();
      console.log(`✓ User "${loginData.firstTimeUser}" confirmed as new user in database`);
    } else {
