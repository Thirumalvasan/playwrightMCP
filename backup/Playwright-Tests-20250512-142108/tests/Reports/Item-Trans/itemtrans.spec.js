...
const itemtransdata_1 = require("../../../testData/itemtransdata");
async function gotoItemTrans(page) {
    ...
    await page.waitForURL(loginData_1.loginData.baseUrl + 'report/itemtransactionlist');
    await (0, test_1.expect)(page).toHaveURL(loginData_1.loginData.baseUrl + 'report/itemtransactionlist');
}
async function handleDateFilterDropdown(page) {
    ...
    for (let i = 0; i < predefinedOptions.length; i++) {
        ...
        await clickDateFilterOption(page, menuLocator, optionText);
        await page.waitForTimeout(3000);
        console.log('Ready for next filter');
    }
    await selectCustomDateRange(page);
}
async function findDateFilterDropdown(page) {
...
async function getDateFilterOptions(page, dropdownToggle) {
    ...
    const predefinedOptions = texts
        .map((t) => t.trim())
        .filter((t) => t.length > 0 && !t.toLowerCase().includes('custom') && !t.toLowerCase().includes('range'));
    return { menuLocator, predefinedOptions };
}
async function reopenDropdownIfNeeded(page, dropdownToggle, menuLocator) {
...
async function clickDateFilterOption(page, menuLocator, optionText) {
...
async function selectCustomDateRange(page) {
    ...
    console.log('Date Range Start:', itemtransdata_1.itemTransData.dateRangeStart);
    console.log('Date Range End:', itemtransdata_1.itemTransData.dateRangeEnd);
    await page.waitForTimeout(2000);
    const dropdownToggle = await findDateFilterDropdown(page);
    await dropdownToggle.click();
    await page.waitForTimeout(1000);
...
async function selectDatesFromFlatpickrCalendar(page) {
    console.log('Selecting dates from Flatpickr calendar...');
    const startDate = new Date(itemtransdata_1.itemTransData.dateRangeStart);
    const endDate = new Date(itemtransdata_1.itemTransData.dateRangeEnd);
...
async function navigateFlatpickrToDate(page, targetDate) {
...
async function navigateFlatpickrByMonth(page, targetDate) {
    const targetMonth = targetDate.getMonth();
    const targetYear = targetDate.getFullYear();
    const currentMonthElem = page.locator('.flatpickr-current-month .cur-month').first();
    if (await currentMonthElem.count() === 0)
        return;
    const currentMonthText = await currentMonthElem.textContent();
    const months = [
        "January", "February", "March", "April", "May", "June",
...
(0, test_1.test)('verify Item Transactions Report page loads correctly', async ({ page }) => {
    test_1.test.setTimeout(180000);
    await gotoItemTrans(page);
    const header = page.getByRole('heading', { name: /Item Transaction/i });
